...
const bold = (s: string) => s.replace(/\*\*(.+?)\*\*/g, "<strong style='color:#e4e4e7'>$1</strong>");
...
${e.tldr.map((t) => `<tr><td style="font-family:Helvetica,Arial,sans-serif;font-size:14px;line-height:1.6;color:#d4d4d8;padding-bottom:6px">→ ${bold(t)}</td></tr>`).join("\n")}
${e.sections
  .map(
    (s) => `
...
${s.paras.map((p) => `<tr><td style="font-family:Helvetica,Arial,sans-serif;font-size:14.5px;line-height:1.65;color:#a1a1aa;padding-bottom:12px">${bold(p)}</td></tr>`).join("\n")}`,
  )
  .join("\n")}
